...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi) //handler async spi read write complete callback
{
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) //handler external callback from DIO_0-5
{
	if(GPIO_Pin == GPIO_PIN_6)	//DIO_0 PACKET SENT PAYLOAD READY
	{

		if(deviceState.modulationType == FSK_OOK_MODULATION)
		{
			if(deviceState.RXTXMode == TX_MODE) //sent packet
			{
			...
			{
				SX1276_77_78_79_readOpMode();
				SX1276_77_78_79_readRSSI();
				SX1276_77_78_79_readIrqFlags_1_2();
				SX1276_77_78_79_readFifo(fifoReadPtr, deviceState.PacketPayloadSize); //return callback for you
			}
		}
		else if(deviceState.modulationType == LORA_MODULATION)
		{
			if(deviceState.RXTXMode == TX_MODE) //sent packet
			{
				...
				deviceState.RXTXMode = RX_MODE;
				SX1276_77_78_79_readLoRaIrqFlags_L();
				SX1276_77_78_79_writeDIOMapping(L_DIO_0_RX_DONE, L_DIO_4_PLL_LOCK | L_DIO_5_CLK_OUT);
			}
			else //packet received and stored in fifo
			{
				SX1276_77_78_79_readLoRaIrqFlags_L();
				SX1276_77_78_79_writeLoRaIrqFlags_L(L_RX_DONE_MASK | L_VALID_HEADER_MASK); //clear rx_done flag and vh flag
				deviceState.LoraPacketReceivingOnGoing = 1;
				SX1276_77_78_79_readFifoRxBytesNb_L(); //start read fifo sequence
			}
		}
	}
}
...
void SX1276_77_78_79_LORA_Init(uint8_t nodeAddress, uint8_t broadcastAddress, uint8_t payloadSize, float frequency_kHz, uint8_t spreadingFactor)
{
	...
	registersConfig.RegLna = LNA_GAIN_HIGHEST_GAIN | LNA_BOOST_LF_DEFAULT | LNA_BOOST_HF_DEFAULT;
	...
	registersConfig.RegOpMode = LONG_RANGE_MODE_LORA_MODE | MODE_SLEEP_MODE | LOW_FREQ_MODE_ON_LOW_FREQ | L_ACCESS_SHARED_REG;
...
uint8_t writeToRegisterSync(uint8_t address, uint8_t value)
{
	...
	sendBuf[1] = value;
	SPI1_CS_ON();
	status = HAL_SPI_Transmit(&hspi1, sendBuf, 2, timeout);
	SPI1_CS_OFF();
	HAL_Delay(1);
	return status;
}
uint8_t readFromRegisterSync(uint8_t address)
{
	...
	sendBuf[1] = 0x00;
	SPI1_CS_ON();
	status = HAL_SPI_Transmit(&hspi1, sendBuf, size, timeout);
	status = HAL_SPI_Receive(&hspi1, receiveData, size, timeout);
	SPI1_CS_OFF();
	HAL_Delay(1);
...
{
	uint8_t result = 0x00;
	result = readFromRegisterSync(REG_OP_MODE_ADDR);
	return result;
}
...
void SPI_dma_request_read_write(SPI_dma_req *req)
{
	...
	*rwqueue_wrptr++ = req;
	...
	if (rwQueueCnt == 0) // Start SPI transfer for first request
	{
		//write first request
		if(hspi1.State == HAL_SPI_STATE_READY)
		{
			SPI1_CS_ON();
			HAL_SPI_TransmitReceive_DMA(&hspi1, req->writeData, req->readData, req->size);
		}
	}
	rwQueueCnt++;
}
...
{
	if(delayTime > HAL_GetTick()) //non blocking delay
	{
		return;
	}
	...
	if(hspi1.State == HAL_SPI_STATE_READY)
	{
		if (rwQueueCnt)
		{
			SPI_dma_req *req = *rwqueue_rdptr++;
			req->ready = 1;
			//может тут всунуть обработку коллбеков так как следующий запрос не будет отправлен пока тут все не обработается
			handleRequestFromSPI(req);

			if (rwqueue_rdptr == rwqueue + SPI_READ_WRITE_QUEUE_SIZE)
			{
				rwqueue_rdptr = rwqueue;
			}
			rwQueueCnt--;
			if(rwQueueCnt) // Start SPI transfer for next request if available
			{
				req = *rwqueue_rdptr;
				//запускаем следующий запрос дма
				SPI1_CS_ON();
				HAL_SPI_TransmitReceive_DMA(&hspi1, req->writeData, req->readData, req->size);
			}
		}
